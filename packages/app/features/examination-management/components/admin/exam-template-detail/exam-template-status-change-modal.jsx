...
import { FileOutlined, CheckCircleOutlined, DeleteOutlined, ClockCircleOutlined, CloseCircleOutlined, CheckOutlined, ExclamationCircleOutlined } from '@ant-design/icons'

/**
 * Modal chuyển trạng thái mẫu đề thi
 ...
 * @param {number} currentStatus - Trạng thái hiện tại của mẫu đề
 */
export function ExamTemplateStatusChangeModal({ open, loading, onCancel, onSubmit, currentStatus }) {
  ...
  return (
    <Modal
      ...
      okButtonProps={{
        ...
        icon: <CheckOutlined />,
        disabled: selectedStatus === null || selectedStatus === currentStatus || !confirmed || (selectedStatus === 2 && deleteConfirm?.toUpperCase() !== 'XÓA'),
      }}
      cancelButtonProps={{
        size: 'middle',
        style: {
          ...
          <>
            {selectedStatus === 2 ? (
              <>
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 14, color: '#991b1b', lineHeight: 1.6 }}>
                      Mẫu đề sẽ được chuyển từ <strong>"{statusOptions.find(s => s.value === currentStatus)?.label}"</strong> sang <strong>"{statusOptions.find(s => s.value === selectedStatus)?.label}"</strong>. 
                      Hành động này có thể ảnh hưởng đến các đề thi đang sử dụng mẫu đề này.
                    </div>
                  </div>
                </div>
                <Form.Item
                ...
                >
                  <Checkbox style={{ fontSize: 14, color: '#374151' }}>
                    Tôi hiểu rõ hậu quả và muốn xóa mẫu đề này
                  </Checkbox>
                </Form.Item>
              </>
            ) : (
              <>
                ...
                >
                  <Checkbox style={{ fontSize: 14, color: '#374151' }}>
                    Tôi xác nhận muốn chuyển trạng thái mẫu đề này
                  </Checkbox>
                </Form.Item>
              </>
            )}
...
export default ExamTemplateStatusChangeModal

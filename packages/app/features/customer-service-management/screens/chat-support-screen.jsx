...
export function ChatSupportScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [pending, mine] = await Promise.all([getPendingSupport(), getMyRooms()])
  ...
  const handleSelectRoom = async (room, isPendingList) => {
    ...
    if (selectedRoom && selectedRoom.id && selectedRoom.id !== room.id) {
      showAdminError('Bạn chỉ được hỗ trợ 1 khách hàng tại một thời điểm. Vui lòng kết thúc phòng hiện tại trước.')
      return
    }

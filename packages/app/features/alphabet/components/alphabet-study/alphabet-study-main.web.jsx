...
export function AlphabetStudyMain({
  ...
  const handleStroke = (path, isEraser) => {
    ...
    const avgDeviation = totalDeviation / userPoints.length // Độ lệch trung bình

    // 2. Template Coverage - user có vẽ phủ hết đường mẫu không
    const sampleCount = Math.max(targetPoints.length * 3, 15) // Nhiều sample hơn để chính xác
    ...
    // avgDeviation = 0 → perfect, avgDeviation >= tolerance → 0 điểm
    const precisionScore = Math.max(0, 1 - (avgDeviation / tolerance))

    // 4. Kiểm tra điểm đầu và điểm cuối
    ...
    const proximityThreshold = 0.12 // 12% - strict hơn
    ...
    if (rawScore >= 90) {
      ...
      // Deviation từ 100, nhân đôi penalty
